...
void test_write(svf::runtime::WriterFn *writer_fn, void *writer_ptr) {
  ...
  schema::Entry entry = {
    ...
    .removeOption1_tag = schema::RemoveOption_tag::one,
    .removeOption1_payload = {
      .one = 70,
    },
    .removeOption2_tag = schema::RemoveOption_tag::two,
    .removeOption2_payload = {
      .two = 71,
    },
    .primitives = {
      .u8u16 = 49,
...
void test_read(vm::LinearArena *arena, Bytes input_range) {
  ...
  ASSERT(entry->addField.one == 68 && entry->addField.two == 0 && entry->addField.three == 69);
