...
Range<Byte> output_bytes(
  ...
  auto out_entry_point = vm::allocate_one<meta::EntryPoint>(output_arena);
	ASSERT(out_entry_point); // temporary
	...
	*out_entry_point = {
		.structs = {
			.pointer_offset = offset_between(out_entry_point, out_structs.pointer),
			.count = safe_int_cast<U32>(out_structs.count),
		},
		.choices = {
			.pointer_offset = offset_between(out_entry_point, out_choices.pointer),
